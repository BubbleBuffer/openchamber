...
describeWithOpenCode("OpenCode tool-part lifecycle (synthetic, provider-agnostic)", { timeoutMs: 60_000 }, (ctx) => {
  ...
  beforeAll(async () => {
    ...
    await fetch(`${ctx.opencode.baseUrl}/session/${sessionId}/prompt_async`, {
      method: "POST",
  ...
  test(
    ...
    async () => {
      const sse = await openSseStream(ctx.opencode.baseUrl)
      try {
