...
// Mirrors tests/web/prompt-async-proxy.test.ts bootstrap:
...
//     matching the actual WebUiServerController type.
...
describeWhenOpenCode("OpenChamber tool-part proxy", () => {
  ...
  test("GET /api/experimental/tool/ids forwards upstream response", async () => {
    const res = await fetch(`${webBaseUrl}/api/experimental/tool/ids`, { method: "GET" })
  ...
  test("PATCH /api/session/.../part/{partID} forwards ToolPart update", async () => {
    ...
    const res = await fetch(
      `${webBaseUrl}/api/session/${sessionId}/message/${messageId}/part/${partID}`,
      {
        method: "PATCH",
