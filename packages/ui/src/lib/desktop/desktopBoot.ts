 ...
 *
 * Desktop shells must wait until a valid boot outcome is injected by the main process.
 ...
 * `/api/*` requests cannot recover the app, so callers must restart the shell.
 */
...
/**
 * Read the boot outcome injected by the desktop main process.
