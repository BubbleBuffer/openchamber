...
// Re-export selectors from the machine package
export {
  selectIsStreaming,
  selectIsWorking,
...
export type { SessionRetrySnapshot, SessionHistorySnapshot } from '@openchamber/session-state'
...
export function useHistoryState(directory: string, sessionId: string): { isLoadingOlder: boolean; hasMoreAbove: boolean; oldestLoadedMessageId: string | null; newestLoadedMessageId: string | null; historyLoadError: string | null } {
  return useActorSelector(directory, sessionId, (snap) => {
    const ctx = snap.context as SessionMachineContext | undefined
...
export function useFatalError(directory: string, sessionId: string): { invariantName: string; actorKey: string; revision: number; eventType: string } | null {
