...
export type { EventRoutingIndex } from "./event-pipeline"
...
export function handleEvent(
  ...
  try {
    ...
    reportError(error, { action: "Apply live session event", scope: "sync-event-handler", silent: true })
  }
    ...
    const triggerRecoveryResync = (directory: string) => {
      ...
      void resyncDirectoryAfterReconnect(directory, store, routingIndex)
        .catch((error) => {
          ...
          reportError(error, { action: "Resync session state", scope: "sync-resync", silent: true });
        })
        .finally(() => {
