...
const PROMPT_TEMPLATE_DEFINITIONS: readonly PromptTemplateDefinition[] = [
  {
...
const PROMPT_TEMPLATE_BY_ID = new Map<PromptTemplateId, PromptTemplateDefinition>(
  PROMPT_TEMPLATE_DEFINITIONS.map((definition) => [definition.id, definition])
);
...
function getPromptTemplateDefinition(id: PromptTemplateId): PromptTemplateDefinition {
  const definition = PROMPT_TEMPLATE_BY_ID.get(id);
  if (!definition) {
...
export function renderPromptTemplate(
  ...
  return replaceTemplateVariables(getPromptTemplateDefinition(id).template, variables);
}
