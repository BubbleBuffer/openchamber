...
- The UI resolves an icon id in `packages/web/src/ui/lib/files/fileTypeIcons.ts`.
- Valid icon ids are loaded from `packages/web/src/ui/lib/files/fileTypeIconIds.ts`.
- `packages/web/src/ui/components/icons/FileTypeIcon.tsx` renders the icon with `<use href="...#icon-id" />` from `sprite.svg`.
...
The command regenerates both `sprite.svg` and `packages/web/src/ui/lib/files/fileTypeIconIds.ts`.
