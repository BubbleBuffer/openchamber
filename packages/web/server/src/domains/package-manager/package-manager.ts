...
let cachedDetectedPm: string | null = null;

function getSpawnSyncBaseOptions(): object {
...
function getOpenChamberConfigDir(): string {
  if (process.platform === "win32") {
    const appData = process.env.APPDATA;
    if (appData) return path.join(appData, "openchamber");
  }

  return path.join(homedir(), ".config", "openchamber");
}
...
function normalizeDeviceClass(value: string): string {
  if (
    value === "mobile" ||
    value === "tablet" ||
    value === "desktop" ||
...
): Promise<CheckForUpdatesFromApiResult | null> {
  try {
    ...
    return {
      ...
      nextSuggestedCheckInSec:
        ...
        Number.isFinite(data.nextSuggestedCheckInSec)
...
function getPackagePathForGlobalRoot(rootPath: string): string | null {
...
function getGlobalBinDirs(pm: string): string[] {
...
function getGlobalNodeModulesRoots(pm: string): string[] {
  try {
    const pmCommand = resolvePackageManagerCommand(pm);
    if (!isCommandAvailable(pmCommand)) {
      return [];
    ...
    switch (pm) {
      case "pnpm": {
        ...
        if (pnpmPrefix)
          roots.push(
            process.platform === "win32"
              ? path.join(pnpmPrefix, "node_modules")
              : path.join(pnpmPrefix, "lib", "node_modules")
          );
        break;
      }
      case "yarn": {
...
function getOwnedPackagePathsFromGlobalBins(pm: string): string[] {
  const packagePaths: string[] = [];
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName =
      process.platform === "win32" ? "openchamber.cmd" : "openchamber";
    const binaryPath = path.join(binDir, binaryName);
    if (!existsSync(binaryPath)) continue;

    try {
      const realBinaryPath = fs.realpathSync.native
        ? fs.realpathSync.native(binaryPath)
...
function detectPackageManagerFromCurrentInstallPath(): string | null {
  return detectPackageManagerFromInstallPath(getCurrentPackagePath());
}

function packageManagerOwnsCurrentInstall(pm: string): boolean {
...
export function detectPackageManagerDetails(): PackageManagerInfo {
    ...
    if (isCommandAvailable(forcedPmCommand)) {
      ...
      return {
        ...
        globalNodeModulesRoot:
          getGlobalNodeModulesRoots(cachedDetectedPm)[0] || null,
      };
    }
  }
  ...
  if (
    hintedPm &&
    isCommandAvailable(resolvePackageManagerCommand(hintedPm)) &&
    isPackageInstalledWith(hintedPm)
  ) {
  ...
  if (
    runtimePm &&
    isCommandAvailable(resolvePackageManagerCommand(runtimePm)) &&
    isPackageInstalledWith(runtimePm)
  ) {
  ...
  const pmChecks = [
    {
      name: "pnpm",
      check: () => isCommandAvailable(resolvePackageManagerCommand("pnpm")),
    },
    {
      name: "yarn",
      check: () => isCommandAvailable(resolvePackageManagerCommand("yarn")),
    },
    {
      name: "bun",
      check: () => isCommandAvailable(resolvePackageManagerCommand("bun")),
    },
    {
      name: "npm",
      check: () => isCommandAvailable(resolvePackageManagerCommand("npm")),
    },
  ];
...
function detectPackageManagerFromInstallPath(pkgPath: string): string | null {
  ...
  if (normalized.includes("/.bun/") || normalized.includes("/bun/install/"))
    return "bun";
  if (normalized.includes("/node_modules/")) return "npm";
  return null;
}

function detectPackageManagerFromRuntimePath(runtimePath: string): string | null {
  if (!runtimePath || typeof runtimePath !== "string") return null;
  const normalized = runtimePath.replace(/\\/g, "/").toLowerCase();
  if (
    normalized.includes("/.bun/bin/bun") ||
...
function detectPackageManagerFromInvocationPath(invokedPath: string): string | null {
...
function getPackageManagerCommandCandidates(pm: string): string[] {
  ...
  if (pm === "bun") {
    ...
    if (process.env.USERPROFILE) {
      candidates.push(
        path.join(process.env.USERPROFILE, ".bun", "bin", bunExecutable)
      );
    }
  }
  candidates.push(pm);
...
function resolvePackageManagerCommand(pm: string): string {
...
function isPackageInstalledWith(pm: string): boolean {
  try {
    const pmCommand = resolvePackageManagerCommand(pm);
    let args: string[];
    switch (pm) {
    ...
    return (
      result.stdout.includes(PACKAGE_NAME) ||
 ...
 */
export function getUpdateCommand(pm = detectPackageManager()): string {
