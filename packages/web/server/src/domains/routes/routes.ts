...
export function registerOpenCodeRoutes(app: any, deps: OpenCodeRoutesDeps): void {
  ...
  app.get('/api/config/settings', async (_req: any, res: any) => {
    try {
  ...
  app.get('/api/config/opencode-resolution', async (_req: any, res: any) => {
    try {
      const settings = await readSettingsFromDiskMigrated();
  ...
  app.post('/api/mcp/auth/pending', async (req: any, res: any) => {
    try {
      ...
      return res.status(500).json({ error: (error as Error).message || 'Failed to store pending MCP auth context' });
    }
  });

  app.get('/api/mcp/auth/pending', async (req: any, res: any) => {
    try {
      pruneExpiredPendingMcpAuthContexts();
      ...
      return res.status(500).json({ error: (error as Error).message || 'Failed to read pending MCP auth context' });
    }
  });

  app.delete('/api/mcp/auth/pending', async (req: any, res: any) => {
    try {
      const state = normalizePendingString(Array.isArray(req.query?.state) ? req.query.state[0] : req.query?.state);
      if (!state) {
      ...
      return res.status(500).json({ error: (error as Error).message || 'Failed to clear pending MCP auth context' });
    }
  });

  app.get('/api/provider/:providerId/source', async (req: any, res: any) => {
    try {
      ...
      return res.status(500).json({ error: (error as Error).message || 'Failed to get provider sources' });
    }
  });

  app.delete('/api/provider/:providerId/auth', async (req: any, res: any) => {
    try {
      const { providerId } = req.params;
      if (!providerId) {
        return res.status(400).json({ error: 'Provider ID is required' });
      ...
      return res.status(500).json({ error: (error as Error).message || 'Failed to disconnect provider' });
    }
  });

  app.post('/api/opencode/directory', async (req: any, res: any) => {
    try {
      ...
      const existingProjects = sanitizeProjects((currentSettings as any).projects) || [];
      ...
      return res.status(500).json({ error: (error as Error).message || 'Failed to update working directory' });
    }
  });
}