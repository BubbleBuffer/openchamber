...
export function registerGitRoutes(app: Express): void {
  ...
  app.post("/api/git/set-identity", async (req, res) => { try { const dir = directory(req, res); const body = request<any>("POST /api/git/set-identity", req.body, res); if (!dir || !body) return; const git = await getGitLibraries(); const selected = body.profileId === "global" ? await git.getGlobalIdentity() : git.getProfile(body.profileId); if (!selected || !selected.userName || !selected.userEmail) return failure(res, new Error("Profile not found"), "Git identity profile missing:"); const profile = { id: body.profileId, name: "name" in selected ? selected.name : "Global Identity", userName: selected.userName, userEmail: selected.userEmail, sshKey: "sshKey" in selected ? selected.sshKey ?? null : "sshCommand" in selected ? sshKeyFromCommand(selected.sshCommand) : null }; await git.setLocalIdentity(dir, profile); return respond("POST /api/git/set-identity", res, { success: true, profile }); } catch (error) { return failure(res, error, "Failed to set git identity:"); } });
  ...
  app.post("/api/git/rebase", operation("POST /api/git/rebase", async (_req, dir, body, git) => git.rebase(dir, body)));
  app.post("/api/git/rebase/abort", emptyOperation("POST /api/git/rebase/abort", async (_req, dir, git) => git.abortRebase(dir)));
  app.post("/api/git/merge", operation("POST /api/git/merge", async (_req, dir, body, git) => git.merge(dir, body)));
  app.post("/api/git/merge/abort", emptyOperation("POST /api/git/merge/abort", async (_req, dir, git) => git.abortMerge(dir)));
  app.post("/api/git/rebase/continue", emptyOperation("POST /api/git/rebase/continue", async (_req, dir, git) => git.continueRebase(dir)));
  app.post("/api/git/merge/continue", emptyOperation("POST /api/git/merge/continue", async (_req, dir, git) => git.continueMerge(dir)));
