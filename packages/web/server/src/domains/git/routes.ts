...
export function registerGitRoutes(app: Express): void {
  ...
  app.post("/api/git/set-identity", async (req, res) => { try { const dir = directory(req, res); const body = request<any>("POST /api/git/set-identity", req.body, res); if (!dir || !body) return; const git = await getGitLibraries(); const selected = body.profileId === "global" ? await git.getGlobalIdentity() : git.getProfile(body.profileId); if (!selected || !selected.userName || !selected.userEmail) return failure(res, new Error("Profile not found"), "Git identity profile missing:"); const profile = { id: body.profileId, name: "name" in selected ? selected.name : "Global Identity", userName: selected.userName, userEmail: selected.userEmail, sshKey: "sshKey" in selected ? selected.sshKey ?? null : "sshCommand" in selected ? selected.sshCommand ?? null : null }; await git.setLocalIdentity(dir, profile); return respond("POST /api/git/set-identity", res, { success: true, profile }); } catch (error) { return failure(res, error, "Failed to set git identity:"); } });
