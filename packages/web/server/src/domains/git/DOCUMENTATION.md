...
- `packages/web/server/src/domains/git/`: Git module directory containing all Git-related functionality.
  - `index.ts`: Public API entry point imported by the server composition root.
...
1. Add the function to `packages/web/server/src/domains/git/service.ts`.
