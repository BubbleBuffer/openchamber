...
export const createOpenCodeLifecycleRuntime = (deps: any) => {
  const {
    state,
    env,
    syncToHmrState,
  ...
  const hasChildProcessExited = (child: any): boolean =>
    !child || child.exitCode !== null || child.signalCode !== null;

  const waitForChildProcessClose = (
    child: any,
    timeoutMs: number
  ): Promise<boolean> => {
  ...
  const closeManagedOpenCodeChild = async (child: any): Promise<void> => {
    if (!child) {
      return;
    }
  ...
  const createManagedOpenCodeServerProcess = async ({
    ...
    if (process.platform === "win32" && state.useWslForOpencode) {
      const wslBinary =
        state.resolvedWslBinary || resolveWslExecutablePath();
      if (!wslBinary) {
        throw new Error(
          "WSL executable not found while attempting to launch OpenCode from WSL"
        );
      }

      const wslOpencode =
        state.resolvedWslOpencodePath &&
        state.resolvedWslOpencodePath.trim().length > 0
      ...
      args = buildWslExecArgs(
        [wslOpencode, "serve", "--hostname", serveHost, "--port", String(port)],
    ...
    if (process.platform === "win32" && !state.useWslForOpencode) {
      ...
      if (launchSpec?.binary) {
        if (launchSpec.wrapperType) {
          console.log(
            `Launching OpenCode via ${launchSpec.wrapperType}: ${launchSpec.binary}`
          );
        }
    ...
    const url = await new Promise<string>((resolve, reject) => {
      let output = "";
      let done = false;
      const finish = (handler: (val: any) => void, value: any) => {
        if (done) return;
        done = true;
        clearTimeout(timer);
      ...
      const onExit = (code: any) => {
        finish(
          reject,
          new Error(`OpenCode exited with code ${code}. Output: ${output}`)
        );
      };
  ...
  const isOpenCodeProcessHealthy = async (): Promise<boolean> => {
  ...
  const probeExternalOpenCode = async (
    port: number,
    origin?: string
  ): Promise<boolean> => {
    if (!port || port <= 0) {
  ...
  const waitForOpenCodePort = async (
    timeoutMs = 15000
  ): Promise<number> => {
  ...
  const startOpenCode = async (): Promise<ManagedChild> => {
    const desiredPort = env.ENV_CONFIGURED_OPENCODE_PORT ?? 0;
    const spawnPort = await resolveManagedOpenCodePort(
      desiredPort,
      env.ENV_CONFIGURED_OPENCODE_HOSTNAME
    );
    console.log(
      desiredPort > 0
    ...
    const envPath =
      typeof buildManagedOpenCodePath === "function"
        ? buildManagedOpenCodePath()
  ...
  const restartOpenCode = async (): Promise<void> => {
    ...
    state.currentRestartPromise = (async () => {
      ...
      if (state.isExternalOpenCode) {
        ...
        const probePort =
          state.openCodePort || env.ENV_CONFIGURED_OPENCODE_PORT || 4096;
        const probeOrigin =
        ...
        if (healthy) {
          console.log(
            `External OpenCode server on port ${probePort} is healthy`
          );
      ...
      if (!(await waitForPortRelease(portToKill, 5000))) {
        console.warn(
          `Timed out waiting for OpenCode port ${portToKill} to be released`
        );
      }

      if (env.ENV_CONFIGURED_OPENCODE_PORT) {
        console.log(
          `Using OpenCode port from environment: ${env.ENV_CONFIGURED_OPENCODE_PORT}`
        );
    ...
    try {
      ...
      console.error(
        `Failed to restart OpenCode: ${error?.message ?? String(error)}`
      );
  ...
  const waitForOpenCodeReady = async (
    ...
    if (lastError) {
      state.lastOpenCodeError = (lastError as Error).message || String(lastError);
      throw lastError;
    }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (
            ...
            agents.some((agent: any) => agent?.name === agentName)
          ) {
            return;
          }
        }
  ...
  const refreshOpenCodeAfterConfigChange = async (
    reason: string,
    options: any = {}
  ): Promise<void> => {
    ...
    try {
      ...
      state.openCodeNotReadySince = Date.now();
      console.error(
        `Failed to refresh OpenCode after ${reason}: ${error?.message}`
      );
      throw error;
    }
  };
  ...
  const bootstrapOpenCodeAtStartup = async (): Promise<void> => {
    try {
      ...
      if (await isOpenCodeProcessHealthy()) {
        ...
        console.log(
          `Using external OpenCode server at ${label} (skip-start mode)`
        );
        state.openCodeBaseUrl =
          env.ENV_CONFIGURED_OPENCODE_HOST?.origin ?? null;
        ...
        !env.ENV_EFFECTIVE_PORT &&
        (await probeExternalOpenCode(4096))
      ) {
        console.log(
          "Auto-detected existing OpenCode server on default port 4096"
        );
  ...
  const triggerHealthCheck = async (): Promise<void> => {
    if (
    ...
    try {
      ...
      if (!healthy) {
        console.log(
          "[lifecycle] immediate health check: OpenCode not healthy, restarting..."
        );
        await restartOpenCode();
      }
    } catch (error: any) {
      console.error(
        `[lifecycle] immediate health check error: ${error?.message}`
      );
    }
  };

  const startHealthMonitoring = (intervalMs: number): void => {
