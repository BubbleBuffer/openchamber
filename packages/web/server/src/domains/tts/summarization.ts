import type { TtsSummarizeOptions, TtsSummarizeResult } from "./types.js";
...
function sanitizeByMode(text: string, mode: string): string {
...
interface ZenOutput {
  output?: Array<ZenOutputItem | null>;
}
...
function distillNoteFallback(text: string, maxLength: number): string {
  ...
  const best = (sentences[0] || normalized)
    ...
    .split(/,\s+/)[0]
    .trim();
  const idealLimit = Math.min(
    maxLength,
    Math.max(32, Math.floor(normalized.length * 0.65)),
  );
...
export async function summarizeText(
  opts: TtsSummarizeOptions,
): Promise<TtsSummarizeResult> {
  const { text, threshold = 200, maxLength = 500, zenModel, mode = "tts" } = opts;
  ...
  try {
    ...
    if (summary) {
      ...
      const finalSummary =
        mode === "note"
          ? sanitized && sanitized !== sanitizeForNote(text)
            ? sanitized
            : distillNoteFallback(text, maxLength)
          : sanitized;
      return {
...
export { summarizeText as default };
