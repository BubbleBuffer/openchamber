...
import type {
  InstallSkillsFromClawdHubResult,
  ClawdHubInstallResult,
  ClawdHubInstallError,
} from "../types.js";

const SKILL_NAME_PATTERN = /^[a-z0-9][a-z0-9-]*[a-z0-9]$|^[a-z0-9]$/;

function normalizeUserSkillDir(userSkillDir: string | null): string | null {
  ...
  const pluralSkillDir = path.join(
    os.homedir(),
    ".config",
    "opencode",
    "skills"
  ...
  if (userSkillDir === legacySkillDir) {
    if (fs.existsSync(legacySkillDir) && !fs.existsSync(pluralSkillDir))
...
function getTargetSkillDir({
  ...
  if (scope === "user") {
    ...
    if (!userSkillDir) {
      throw new Error("userSkillDir is required for user scope");
    }
...
export async function installSkillsFromClawdHub({
...
} = {}): Promise<InstallSkillsFromClawdHubResult> {
  ...
  if (
    ...
    return {
      ok: false,
      error: { kind: "invalidSource", message: "Invalid target source" },
    };
  }
  ...
  const skillPlans = requestedSkills.map((sel) => {
    ...
    return {
      ...
      installable: validateSkillName(String(slug)),
    };
  });
  ...
  const installed: Array<{ skillName: string; scope: string; source: string }> = [];
  ...
  for (const plan of skillPlans) {
    ...
    try {
      ...
      if (resolvedVersion === "latest") {
        try {
          const info = (await fetchClawdHubSkillInfo(plan.slug)) as {
            skill?: { tags?: { latest?: string } };
            latestVersion?: { version?: string };
          };
          const latest =
            info.skill?.tags?.latest || info.latestVersion?.version || null;
          if (latest) {
        ...
        if (resolvedVersion === "latest") {
          skipped.push({
            skillName: plan.slug,
            reason: "Unable to resolve latest version",
          });
          continue;
        }
      }
      ...
      const tempDir = await fs.promises.mkdtemp(
        path.join(os.tmpdir(), `clawdhub-${plan.slug}-`)
      );
      ...
      try {
        ...
        if (!fs.existsSync(skillMdPath)) {
          skipped.push({
            skillName: plan.slug,
            reason: "SKILL.md not found in downloaded package",
          });
          continue;
        }
      ...
      console.error(
        `Failed to install ClawdHub skill "${plan.slug}":`,
        error
      );
      skipped.push({
        skillName: plan.slug,
