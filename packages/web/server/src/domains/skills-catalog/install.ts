...
async function copyDirectoryNoSymlinks(
  srcDir: string,
  dstDir: string
): Promise<void> {
...
): Promise<CloneSuccess | CloneFailure> {
  const { cloneUrl, identity, tempDir } = options;
  const preferred = [
    "clone",
    "--depth",
    "1",
...
export async function installSkillsFromRepository({
...
} = {}): Promise<InstallSkillsFromRepositoryResult> {
  const gitCheck = await assertGitAvailable();
  if (!gitCheck.ok) {
    return { ok: false, error: gitCheck.error };
  }

  const normalizedUserSkillDir = normalizeUserSkillDir(userSkillDir || null);
  ...
  try {
    ...
    await runGit(
      ["-C", tempBase, "sparse-checkout", "init", "--cone"],
      { identity, timeoutMs: 15_000 }
    );
    ...
    if (!setResult.ok) {
      return {
        ...
        error: {
          kind: "unknown",
          message:
            setResult.stderr || setResult.message || "Failed to configure sparse checkout",
        },
      };
    }
