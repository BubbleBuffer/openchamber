...
export async function scanSkillsRepository({
...
} = {}): Promise<ScanSkillsRepositoryResult> {
  ...
  const tempBase = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "openchamber-skills-scan-")
  );
  ...
  try {
    ...
    const toFsPath = (posixPath: string): string =>
      path.join(
        tempBase,
        ...String(posixPath || "").split("/").filter(Boolean)
      );
    ...
    const sparseInit = await runGit(
      ["-C", tempBase, "sparse-checkout", "init", "--no-cone"],
      { identity, timeoutMs: 15_000 }
    );
    if (sparseInit.ok) {
      const sparseSet = await runGit(
        ["-C", tempBase, "sparse-checkout", "set", ...patterns],
    ...
    const worker = async (): Promise<void> => {
      while (idx < uniqueSkillDirs.length) {
        ...
        const frontmatterName =
          typeof parsedMd.frontmatter?.name === "string"
        ...
        if (!installable) {
          warnings.push(
            "Skill directory name is not a valid OpenCode skill name"
          );
        }
    ...
    await Promise.all(
      Array.from({ length: Math.min(maxParallel, uniqueSkillDirs.length || 1) }, () =>
        worker()
      )
    );
