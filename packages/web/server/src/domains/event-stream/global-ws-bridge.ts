...
import type { GlobalWsBridge, GlobalWsBridgeDeps, NormalizedEvent } from "./types.js";
...
export function createGlobalMessageStreamWsBridge({
  ...
  triggerHealthCheck,
  heartbeatIntervalMs,
}: GlobalWsBridgeDeps): GlobalWsBridge {
  const clients = new Set<WebSocket>();
  const clientLastEventIds = new Map<WebSocket, string>();
  const readyClients = new Set<WebSocket>();

  const removeClient = (socket: WebSocket) => {
  ...
  const replayEvents = (socket: WebSocket, requestedLastEventId: string) => {
  ...
  const markReady = (socket: WebSocket, requestedLastEventId: string) => {
  ...
  const closeClientsWithInitialError = ({
    message,
    closeReason = message,
  ...
  const unsubscribeEvent = globalHub.subscribeEvent(({ payload, directory, eventId }: NormalizedEvent) => {
    for (const socket of Array.from(clients)) {
  ...
  const accept = (socket: WebSocket, { requestedLastEventId = "" }: { requestedLastEventId?: string } = {}) => {
