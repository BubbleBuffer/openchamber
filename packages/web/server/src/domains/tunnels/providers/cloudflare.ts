...
import type { TunnelProviderCapabilities, TunnelProvider, CheckResult, TunnelController, TunnelStartRequest } from "../types.js";
...
interface DescribedMode {
  mode: string;
  checks: CheckResult[];
  summary: ModeCheckResult;
  ready: boolean;
  blockers: string[];
}
...
type DiagnoseFn = (request?: Record<string, unknown>) => Promise<DiagnoseResult>;

export function createCloudflareTunnelProvider(): TunnelProvider & { diagnose: DiagnoseFn } {
  const validateTokenShape = (value: unknown): { ok: boolean; detail: string } => {
  ...
  const createModeSummary = (checks: CheckResult[]): ModeCheckResult => {
  ...
  const describeMode = ({ mode, checks }: { mode: string; checks: CheckResult[] }): DescribedMode => {
  ...
  return {
    ...
    diagnose: async (request: Record<string, unknown> = {}) => {
      ...
      const managedLocalInspection = inspectManagedLocalCloudflareConfig({
        configPath: request.configPath as string | undefined,
        hostname: request.hostname as string | undefined,
      });
      const managedLocalChecks: CheckResult[] = [
        {
          id: "startup_readiness",
          label: "Provider startup readiness",
          status: startupReady ? "pass" : "fail",
      ...
      const tokenMissing = typeof request.token !== "string" || (request.token as string).trim().length === 0;
      ...
      const managedRemoteChecks: CheckResult[] = [
        {
          id: "startup_readiness",
          label: "Provider startup readiness",
          status: startupReady ? "pass" : "fail",
      ...
      const modeFilter =
        typeof request.mode === "string" && request.mode.trim().length > 0
    ...
    start: async (
      ...
      context: { originUrl?: string; activePort?: number | null },
    ) => {
      ...
      if (request.mode === TUNNEL_MODE_MANAGED_LOCAL) {
        return startCloudflareManagedLocalTunnel({
          configPath: request.configPath ?? undefined,
          hostname: request.hostname,
        });
      }
      ...
      return startCloudflareQuickTunnel({
        ...
        port: context.activePort ?? undefined,
      });
    },
