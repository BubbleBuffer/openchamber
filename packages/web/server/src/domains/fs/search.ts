import type { FsSearchDeps, FsSearchOptions, FsSearchResult, FsSearchRuntime } from "./types.js";
...
function normalizeRelativeSearchPath(
  rootPath: string,
  targetPath: string,
  path: FsSearchDeps["path"],
): string {
  const relative = path.relative(rootPath, targetPath) || path.basename(targetPath);
  return relative.split(path.sep).join("/") || targetPath;
}

function shouldSkipSearchDirectory(name: string, includeHidden: boolean): boolean {
  if (!name) {
    return false;
  }
...
async function listDirectoryEntries(
  dirPath: string,
  fsPromises: FsSearchDeps["fsPromises"],
...
function fuzzyMatchScoreNormalized(normalizedQuery: string, candidate: string): number | null {
...
export const createFsSearchRuntime = ({
...
}: FsSearchDeps): FsSearchRuntime => {
  type Candidate = FsSearchResult & { score: number };
  ...
  const searchFilesystemFiles = async (
    ...
    while (queue.length > 0 && candidates.length < collectLimit) {
      ...
      const dirResults = await Promise.all(
        batch.map(async (dir) => {
          if (!shouldRespectGitignore) {
            return {
              dir,
              dirents: await listDirectoryEntries(dir, fsPromises),
          ...
          try {
            ...
            if (pathsToCheck.length === 0) {
              return { dir, dirents, ignoredPaths: new Set<string>() };
            }
      ...
      for (const { dir: currentDir, dirents, ignoredPaths } of dirResults) {
        for (const dirent of dirents) {
          const entryName: string = dirent.name;
          ...
          const relativePath = normalizeRelativeSearchPath(
            rootPath,
            entryPath,
            path,
          );
    ...
    return candidates.slice(0, limit).map(
      ({ name, path: filePath, relativePath, extension }) => ({
        name,
        path: filePath,
        relativePath,
        extension,
