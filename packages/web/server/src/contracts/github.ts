...
const reviewComment = (value: unknown) => issueComment(value) && (() => { const input = object(value)!; return optionalString(input.path) && (input.line === undefined || input.line === null || number(input.line)) && (input.position === undefined || input.position === null || number(input.position)); })();
const file = (value: unknown) => { const input = object(value); return !!input && required(input.filename) && optionalString(input.status) && ["additions", "deletions", "changes"].every((key) => input[key] === undefined || number(input[key])) && optionalString(input.patch); };
...
export function parseGitHubPullRequestContextResponse(value: unknown): ParseResult<GitHubPullRequestContextResult> { const input = object(value); const valid = input && typeof input.connected === "boolean" && (!input.connected || ((input.repo === null || repo(input.repo)) && (input.pr === null || pr(input.pr)) && (input.issueComments === undefined || (Array.isArray(input.issueComments) && input.issueComments.every(issueComment))) && (input.reviewComments === undefined || (Array.isArray(input.reviewComments) && input.reviewComments.every(reviewComment))) && (input.files === undefined || (Array.isArray(input.files) && input.files.every(file))) && optionalString(input.diff) && (input.checks === undefined || input.checks === null || checks(input.checks)) && (input.checkRuns === undefined || (Array.isArray(input.checkRuns) && input.checkRuns.every(checkRun))))); return valid ? { ok: true, value: input as GitHubPullRequestContextResult } : invalid("invalid GitHub PR context"); }
