...
export type OwnerSessionResponse = { authenticated: boolean; locked?: boolean };
...
export function parseOwnerSessionResponse(value: unknown): ParseResult<OwnerSessionResponse> {
  ...
  if (!authenticated.ok || (object.value.locked !== undefined && !parseJsonBoolean(object.value.locked).ok)) return invalid("invalid owner session response");
  return { ok: true, value: { authenticated: authenticated.value, ...(typeof object.value.locked === "boolean" ? { locked: object.value.locked } : {}) } };
}
